...
from model.causal_module import align_loss
    ...
    for i_batch, batch_dict in enumerate(
        ...
        if args.vg_loss:
            ...
            for aid in range(len(text)):  # one forward per answer candidate id
                ...
                vfeat_list.append(vfeat)
                wfeat_list.append(wfeat)
            vfeats = torch.stack(vfeat_list, 1)
            wfeats = torch.stack(wfeat_list, 1)
            ...
            if data_loader.dataset.mc > 1:
                ...
                try:
                    ...
                    print('')
                vg_loss = (pos_loss + qneg_loss) / 2 + 0.5*align_loss(vfeats, wfeats, qgt) # balanced BCE
            else:
                vg_loss = F.binary_cross_entropy(qlogits.squeeze(1), qgt.float()) + 0.5*align_loss(vfeats, wfeats, qgt)
        ...
        if data_loader.dataset.mc > 1:
            ...
            vqa_loss = (pos_loss + neg_loss) / 2 + 0.5*align_loss(vfeats, wfeats, gt) # balanced BCE
        else:
            vqa_loss = F.binary_cross_entropy(logits.squeeze(1), gt.float()) + 0.5*align_loss(vfeats, wfeats, gt) 
...
def evaluate(
    ...
    if isinstance(next(iter(results.values())), dict):
        ...
        return results, acc, [ground_res, gauss_masks] 
    else:
        return results, 0, [ground_res, gauss_masks]
...
if __name__ == "__main__":
    ...
    args.model_name = './data/pretrain_models/deberta-v2-xlarge'
    main(args)
