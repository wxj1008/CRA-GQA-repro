...
from .causal_module import GroundingModule, Intervention
...
class DebertaV2ForMaskedLM(DebertaV2PreTrainedModel):
    ...
    def __init__(
        ...
        pt_config_name = '/GLOBALFS/sysu_liuyang_1/chenweixing/project/gqa/NExT-GQA/code/FrozenGQA/data/pretrain_models/deberta-v2-xlarge'
    ...
    def forward(
        ...
        # print("input_ids", input_ids.size(), "video", video.size()) # input_ids torch.Size([6, 38]) video torch.Size([6, 32, 768])
        ...
        # print("last_hidden", last_hidden.size()) # last_hidden torch.Size([6, 70, 1536])

        vt_mask = torch.cat([video_mask, attention_mask], dim=1)

        trans = self.trans_gauss(x=last_hidden, attn_mask=vt_mask)[0]   
        # print("trans", trans.size()) trans torch.Size([6, 70, 1536])
        ...
        # print("mask_token_rep", mask_token_rep.size()) # mask_token_rep torch.Size([6, 1536])

        logits_cw = self.logit_gauss(mask_token_rep).view(-1, 2)
        # print("logits_cw", logits_cw.size()) # logits_cw torch.Size([6, 2])
        ...
        # print("vfeat", vfeat.size()) # vfeat torch.Size([6, 32, 1536])
        ...
        # print("new_last_hidden", new_last_hidden.size()) # new_last_hidden torch.Size([6, 70, 1536])

        prediction_scores = self.lm_predictions(new_last_hidden, embeddings, bias)
        # print("prediction_scores", prediction_scores.size()) # prediction_scores torch.Size([6, 70, 2])
        # print(labels)
...
# our model
##############################################################################################
class CausalDebertaV2ForMaskedLM(DebertaV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"predictions.decoder.bias"]
    ...
    def __init__(
        ...
        pt_config_name = './data/pretrain_models/deberta-v2-xlarge'
        config_light = config.from_pretrained(
            pt_config_name,
            num_hidden_layers=1,
            hidden_size=self.deberta.embeddings.embedding_size,
    ...
    def forward(
        ...
        inds = (input_ids == tokenizer.mask_token_id).nonzero(as_tuple=False)[:,1]
        mask_token_rep = last_hidden[torch.arange(last_hidden.shape[0]), self.max_feats+inds, :]  # [B, D]
        
        x_phi = mask_token_rep
        # NOTE Lin Causal
        wfeat = last_hidden[:, self.max_feats:] # [B, M, D]
        mask = torch.ones(wfeat.size(0), wfeat.size(1), wfeat.size(2), dtype=torch.bool).to(wfeat.device)
        mask[torch.arange(wfeat.shape[0]), inds, :] = 0
        wfeat = wfeat.masked_select(mask).view(wfeat.size(0), -1, wfeat.size(2))

        wfeat_attn = self.word_attn(wfeat)
        wfeat = torch.sum(wfeat*wfeat_attn, dim=1) # [B, D]
        x_dot = self.fusion_lin(torch.cat([x_phi, wfeat], dim=-1))
        # x_dot = self.do.lin_do(x_dot)

        # NOTE Get Time Param
        vfeat = last_hidden[:, : self.max_feats]
        time_param = self.grounding_module(vfeat, x_dot)
        
        # NOTE Vis Causal
        trans = self.trans(x=vfeat, attn_mask=video_mask)[0]   
        x_star = self.trans(x=vfeat, attn_mask=video_mask)[0]
        x_bar = torch.sum(trans*time_param["key"].unsqueeze(dim=-1), dim=1) # [B, D]
        x_star = torch.mean(trans, dim=1)
        x_bar = self.fusion(torch.cat([x_phi, x_bar], dim=-1))
        x_star = self.fusion(torch.cat([x_phi, x_star], dim=-1))
        prob_x = self.do.vis_do(x_star, x_bar)# .unsqueeze(dim=1)

        # NOTE Make the lasr hidden
        new_trans = torch.cat([trans, last_hidden[:, self.max_feats:]], dim=1)
        new_trans[torch.arange(last_hidden.shape[0]), self.max_feats+inds, :] += prob_x
        new_trans = self.do.vis_norm(new_trans)
        prediction_scores = self.lm_predictions(new_trans, embeddings, bias)
        ...
        return MaskedLMOutput(
            ...
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        ), {'gcenter':time_param["start"], 'gwidth':time_param["end"], 'gweight':time_param["key"], 'vfeat': x_bar, "wfeat": x_dot}
