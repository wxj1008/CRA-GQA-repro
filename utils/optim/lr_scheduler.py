...
def build_lr_scheduler(cfgs, optimizer, len_loader):
    lr_scheduler_fn = cfgs["optim"]["lr_scheduler"]
    step_size = cfgs["optim"]["step_size"]
    epochs = cfgs["optim"]["epochs"]
    if lr_scheduler_fn == 'warmup_steplr':
        lr_scheduler = WarmupAndSteplr(optimizer, step_size * len_loader, epochs * len_loader)
    elif lr_scheduler_fn == 'warmup':
        lr_scheduler = get_linear_schedule_with_warmup(optimizer, step_size * len_loader, epochs * len_loader)
    else:
        lr_scheduler = getattr(torch.optim.lr_scheduler, cfgs["lr_scheduler"])(optimizer, step_size, cfgs["gamma"])
    print(f"Build {lr_scheduler_fn} for {cfgs['optim']['optimizer']} in {cfgs['misc']['running_name']}")
    return lr_scheduler
